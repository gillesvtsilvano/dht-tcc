...
int __init init_module(){
	...
	dht_craft_msg_insert(dst, 10, 6, data);
	dht_craft_msg_remove(dst, 10);
...
void dht_craft_msg_remove(uint8_t* dst, uint8_t key){
	struct sk_buff *skb;
	struct net_device* dev;
	uint8_t hlen, tlen, *p, ret;
...
void dht_craft_msg_retrive(uint8_t* dst, uint8_t key){
...
void dht_insert(uint8_t app, size_t dsize, void* d) {
	...
	uint32_t new_hash = nbt_hash(d, dsize);
...
void dht_print_node(struct dht_node_t* n){
	char* key_data = strsub(n->data, n->key_size);
	printk(KERN_INFO "%s%010u\t%s%u\t%s%s\t%s%s\t%s%s\n",
			"KEY: ", n->key, 
			"TYPE: ", n->type,
			"KEY_DATA: ", key_data,
			"DATA : ", (char*) (n->data + n->key_size),
